...
class xmind_to_xx(object):
    ...
    def __init__(self, xmind_path, xmind_file, name):
        self.xmind_file_path = os.path.join(xmind_path, xmind_file)
        self.data_dict = xmind_to_dict(self.xmind_file_path)
        self.xls_path = os.path.join(xmind_path, 'xls', name + '.xls')
    ...
    def save_xls(self):
        xls_dir = os.path.split(self.xls_path)[0]
...
class style_excel(object):
    ...
    def __init__(self, xls_path, xls_file, name):
        ...
        path = os.path.join(xls_path, xls_file)
        self.name = name
                    ...
                    for o in range(merge_row[0], merge_row[1]):
                        try:
                            for c in range(0, merge_row[2]):
                                if k != None:
                                    break
                                if 'text' in str(self.data.col(c)[o + 1]) and self.end_col(merge_row[0], self.data.col(
                                        merge_row[2] + 1)):
                                    self.do_merge_excel(merge_row[0], merge_row[0] + (r - 1), merge_row[2],
                                                        merge_row[3])
                                    k = 1
                                elif merge_row[0] + r == merge_row[1] and c >= merge_row[2] - 1 and o >= merge_row[
                                    1] - 1:
